...
response.redirect("cst_servico.asp?resp=2")
end if
...
'cod          = UploadRequest.Item("hfcod").Item("Value")
 ...
 NomeServico   = UploadRequest.Item("NomeServico_001").Item("Value")
 anexo         = UploadRequest.Item("anexo").Item("Value")
 TipoServico   = UploadRequest.Item("TipoServico").Item("Value")
...
if botao = "Incluir" then
   ...
   call abreConexao
   sql = "insert into DL_CadServico(NomeServico, Anexo, Tipo_Servico)"
   sql = sql & " values('"&NomeServico&"', '"&arq&"', '"&TipoServico&"')"
	'response.write sql
	'response.end
    conn.execute(sql)
   call fechaConexao
   response.redirect("cst_servico.asp?resp=1")
%>
<%
...
elseif botao = "Alterar" then
 ...
 call abreConexao
  sql = "update DL_CadServico set"
  sql = sql & " NomeServico = '"&NomeServico&"',"
  sql = sql & " Anexo = '"&arq&"',"
  sql = sql & " Tipo_Servico = '"&TipoServico&"'"
  sql = sql & " where id_CadServico = "&cod
   'response.write sql
   'response.end
  conn.execute(sql)
  call fechaConexao
  response.redirect("cst_servico.asp?resp=4")
%>
<%
